...
6. 如需触发动作：
   ...
   - 每次只允许发送一条指令，必须等待本条指令的响应、超时或结果判定完成后，才能发送下一条
   - 严禁任何形式的并行发送、批量发送、交错发送或多通道同时发指令，除非用户明确要求并接受相应风险
...
- 每次发送测试指令必须串行执行：发送一条，观察一条，记录一条，确认结束后再进入下一条。
- 默认禁止并行发送指令；只有用户明确要求并行压测、并发场景复现或多源刺激时，才允许偏离该规则。
