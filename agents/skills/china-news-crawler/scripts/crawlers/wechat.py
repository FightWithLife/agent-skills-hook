...
try:
    import demjson3
except ModuleNotFoundError:  # Optional dependency; only required for WeChat extraction.
    demjson3 = None
...
def _convert_js_obj_to_json(js_obj_str: str) -> str:
    if demjson3 is None:
        raise RuntimeError("demjson3 is required for WeChat parsing but is not installed.")
    try:
