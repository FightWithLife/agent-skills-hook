...
目标是先从仓库中查出 USB 通信所需参数，再写入项目内配置文件，最后复用统一脚本完成探测、打开、发送和收包。
...
如果本轮 USB 动作可能触发复位、重新枚举、进入新状态，或需要结合启动日志判断结果，先开串口抓取。
...
- 任何可能改变设备运行状态的 USB 收发前，都应先开串口抓取。
- 尤其是会触发重启、模式切换、重新枚举或短时状态上报的命令，不要等 USB 动作结束后再补抓串口日志。
- 多条测试指令默认严格串行：发送一条，等待响应、超时或结果判定完成，再发下一条。
